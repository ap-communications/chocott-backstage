...
chocott-backstageの初期状態では Backstageが用意しているサンプルカタログのみが見える状態です。ここにカタログを追加していきましょう。

## 1. 既存のカタログのインポート

最初に chocott-backstage のリポジトリ情報を閲覧できるカタログを登録してみましょう。
...
カタログ情報は、通常それぞれのリポジトリのリポジトリルートにcatalog-info.yaml というファイルを作成し、それを上記の既存カタログのインポートによって取り込みます。  
...
その後、[既存のカタログのインポート](#既存のカタログのインポート)手順と同じように 作成したcatalog-info.yaml のURLで登録してください。  
