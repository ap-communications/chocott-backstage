...
PostgreSQL Databaseを利用する場合、backend.databaseに指定を追加します。clientに `pg` を指定しconnectionにはホスト名やポート、PostgreSQL Serverに接続する際のユーザー名やパスワードを指定します。また、connection poolの指定をする場合はknexConfig.poolにその内容を指定します。
