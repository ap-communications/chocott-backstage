...
「Install」をクリックすると、Callback URLに設定したURLに自動遷移することがありますが、この時点ではBackstageを起動していないためエラーになります。  

![GitHub App redirect page](../images/github-app-redirect-page.png)

![Error Connection Refused](../images/err_connection_refused.png)

その場合は`https://github.com/organizations/<GitHub Organization名>/settings/installations`にアクセスしてください。  
インストール済みのGitHub Appの一覧が表示されている「Installed Actions」のページが表示されます。このリスト内に今回新規作成したGitHub Appが存在していれば、存在していれば、インストールは問題なく完了しています。
