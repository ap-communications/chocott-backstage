...
「Install & Authorize」をクリックすると、Callback URLに設定したURLに自動遷移しますが、この時点ではBackstageを起動していないためエラーになります。  
`https://github.com/organizations/<GitHub Organization名>/settings/installations`にアクセスし、Installed Actionsのリストに新規作成したGitHub Appが存在することを確認してください。  
存在していれば、インストールは問題なく完了しています。
