...
作成したGitHub App登録を行った組織／ユーザーが所有するリポジトリにアクセスすることができます（ orgA という組織に登録した場合は `orgA/repository` に、 userX というパーソナルアカウントに登録した場合は `userX/repository` にアクセス可能となります）。
...
![personal developer setting](./personal-developer-settings.png)
...
![Install App](install-github-app.png)

### 組織アカウントに作成した場合

インストール対象とする組織を選択し「Install」ボタンを実行してください。

### パーソナルアカウントに作成した場合

自分のアカウント名を選択し「Install」ボタンを実行してください。

### リポジトリの選択

インストール時に「All repositories」または「Only select repositories」を選択できます。

**「All repositories」を選択することを推奨します。**
